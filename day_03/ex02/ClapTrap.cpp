...
ClapTrap::ClapTrap() : _hitPoints(100), _maxHPoints(100), _energie(100), _maxEPoints(100), _level(1), _name("No_Name"), _melee(30), _ranged(20), _armor(5)
{
	std::cout << "Oops! " << this->_name << " Constructed." << std::endl;
}

ClapTrap::ClapTrap(std::string name) : _hitPoints(100), _maxHPoints(100), _energie(100), _maxEPoints(100), _level(1), _name(name) , _melee(30), _ranged(20), _armor(5)
{
	std::cout << this->_name << ": Constructed." << std::endl;
}

ClapTrap::ClapTrap(ClapTrap const &src) : _hitPoints(src._hitPoints), _maxHPoints(src._maxHPoints), _energie(src._energie), _maxEPoints(src._maxEPoints), _level(src._level), _name(src._name) , _melee(src._melee), _ranged(src._ranged), _armor(src._ranged)
{
}
